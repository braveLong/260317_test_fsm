...
signal update_hp_bar(hp_bar: int)
...
func calculate_stats() -> void:
	...
	print("my new speed is ", attack_speed)
...
func attack() -> void:
	...
	if state == State.ATTACK:
		return
	state = State.ATTACK
	# Find the attack direction and push to atree blendspace2d
	var mouse_pos: Vector2 = get_global_mouse_position()
	var attack_dir: Vector2 = (mouse_pos - global_position).normalized()
	sprite_2d.flip_h = attack_dir.x < 0 and abs(attack_dir.x) >= abs(attack_dir.y)
